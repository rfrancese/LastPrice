...
public class SportsFragment extends ListFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
    	...
    	int length= db.getOffer("sport").length;
    	scelta=((CatActivity)getActivity()).getAccess();
    	user=((CatActivity)getActivity()).getUser();
    	
     values = new String[length] ;
    	for (int i=0; i<length;i++){
    		values[i]=db.getOffer("sport")[i].getName();
    		descrizione[i]=db.getOffer("sport")[i].getDesc();
    		prezzo[i]=db.getOffer("sport")[i].getPrice();
    		luogo[i]=db.getOffer("sport")[i].getPlace();
    		venditore[i]=db.getOffer("sport")[i].getVenditore();
    		latitudine[i]=db.getOffer("sport")[i].getLat();
    		longitudine[i]=db.getOffer("sport")[i].getLng();
    		
    	
    }  // use your custom layout
    	
    ArrayAdapter<String> adapter = new ArrayAdapter<String>(inflater.getContext(),R.layout.fragment_sports, R.id.sport_text,values );
