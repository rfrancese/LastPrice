...
import database.*;
// Categoria benessere
public class WellnessFragment extends ListFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
    	...
    	int length= db.getOffer("benessere").length;
    	scelta=((CatActivity)getActivity()).getAccess();
    	user=((CatActivity)getActivity()).getUser();
    	
     values = new String[length] ;
    	for (int i=0; i<length;i++){
    		values[i]=db.getOffer("benessere")[i].getName();
    		descrizione[i]=db.getOffer("benessere")[i].getDesc();
    		prezzo[i]=db.getOffer("benessere")[i].getPrice();
    		luogo[i]=db.getOffer("benessere")[i].getPlace();
    		venditore[i]=db.getOffer("benessere")[i].getVenditore();
    		latitudine[i]=db.getOffer("benessere")[i].getLat();
    		longitudine[i]=db.getOffer("benessere")[i].getLng();
    		
    	
    }    // use your custom layout
    	
    ArrayAdapter<String> adapter = new ArrayAdapter<String>(inflater.getContext(),R.layout.fragment_wellness, R.id.label2,values );
    	    setListAdapter(adapter);
        View rootView = inflater.inflate(R.layout.fragment_wellness, container, false);
